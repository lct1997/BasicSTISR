...
class StrokeFocusLoss(nn.Module):
    def __init__(self):
        ...
        stroke_decompose_lines = open('./dataset/english_decomposition.txt',
                     'r').readlines()
        self.dic = {}
    ...
    def build_up_transformer(self):
        ...
        transformer.load_state_dict(torch.load('./dataset/pretrain_transformer_stroke_decomposition.pth'))
        transformer.eval()
    ...
    def forward(self,sr_img, hr_img, label):
        ...
        recognition_loss = -1
        loss = mse_loss + attention_loss * 50
